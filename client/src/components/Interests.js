...
console.log("_________: ", userTags)
    useEffect(() => {
      loadTags();
    ...
    const loadTags = async () => {
      ...
      let currUser = await getUser(user._id, token);
      setCommunitySep({
      ...
      <div>
        <div style={{
                ...
                <div style={styleContainerPl}>
                  {/* to stop undefined shows error */}
                  {(separateByCommunity&& separateByCommunity.place)?separateByCommunity?.place
                    .map((t) => {
                        return(
                ...
                <div style={styleContainerId}>
                  {/* to stop undefined shows error */}
                  {(separateByCommunity && separateByCommunity.identity)?separateByCommunity.identity.map((t) => {
                      return(
                        <TagButton 
                          key = {t._id} 
                          data = {t} 
                ...
                <div style={styleContainerInt}>
                  {/* to stop undefined shows error */}
                  {(separateByCommunity && separateByCommunity.interest)?separateByCommunity.interest.map((t) => {
                    return(
                      <TagButton 
                        key = {t._id} 
                        data = {t} 
